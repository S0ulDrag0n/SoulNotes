...
fn get_config(app: AppHandle) -> AppConfig {
    ...
    // Decrypt API token if it's encrypted in the config file
    ...
    if let Some(ref token) = config.ollama_api_token {
        if !token.is_empty() {
            match decrypt_token(token) {
                Ok(Some(decrypted)) => {
                    // Return the decrypted token for use
                ...
                Err(e) => {
                    log::warn!("Failed to decrypt API token: {}", e);
                    // Keep the original token - it might be plain text
                }
            }
        }
    }
...
// The API token is encrypted before storing in config.yml
#[tauri::command]
fn save_config(app: AppHandle, mut config: AppConfig) -> Result<(), String> {
    // Encrypt the API token before saving to config file
    if let Some(ref token) = config.ollama_api_token {
        if !token.is_empty() {
            match encrypt_token(token) {
                Ok(encrypted) => {
                    config.ollama_api_token = Some(encrypted);
                    log::info!("API token encrypted for storage");
                }
                Err(e) => {
                    log::error!("Failed to encrypt API token: {}", e);
                    return Err(format!("Failed to encrypt API token: {}", e));
                }
            }
        }
    }
...
async fn translate_text(
    ...
    // Default prompt template for translation (same as web version)
...
async fn summarize_text(app: AppHandle, text: String) -> Result<String, String> {
    ...
    // Default prompt template for summarization (same as web version)
...
/// Send a conversation message to Ollama and get AI response
#[tauri::command]
async fn conversation_text(
    ...
    let model = std::env::var("OLLAMA_CONVERSATION_MODEL")
        .ok()
        .or(config.ollama_conversation_model)
        .unwrap_or_else(|| "qwen3.5:latest".to_string());
